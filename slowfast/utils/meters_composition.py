...
class TestMeter(object):
    ...
    def finalize_metrics(self, ks=(1, 5)):
        ...
        if self.multi_label:
            ...
            (
                ...
                self.video_preds_animal,
                self.video_labels_animal,
                self.video_preds_behavior,
                self.video_labels_behavior,
                ks,
            )
    ...
    def finalize_metrics_per_class(self, ks=(1, 5)):
        ...
        if self.multi_label:
            ...
            for animal in unique_animal_labels:
                label_indexs = (self.video_labels_animal == animal).nonzero(
                    as_tuple=True
                )[0]

                sub_label = torch.index_select(
                    self.video_labels_animal, 0, label_indexs
                )
                sub_pred = torch.index_select(self.video_preds_animal, 0, label_indexs)

                num_topks_correct_animal = metrics.topks_correct(
                    sub_pred, sub_label, ks
                )

                topks_animal = [
                    (x / sub_pred.size(0)) * 100.0 for x in num_topks_correct_animal
                ]
            ...
            for k, topk in zip(ks, animal_perclass_acc):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["perclass_animal_top{}_acc".format(k)] = (
            ...
            for behavior in unique_behavior_labels:
                label_indexs = (self.video_labels_behavior == behavior).nonzero(
                    as_tuple=True
                )[0]

                sub_label = torch.index_select(
                    self.video_labels_behavior, 0, label_indexs
                )
                sub_pred = torch.index_select(
                    self.video_preds_behavior, 0, label_indexs
                )

                num_topks_correct_behavior = metrics.topks_correct(
                    sub_pred, sub_label, ks
                )

                topks_behavior = [
                    (x / sub_pred.size(0)) * 100.0 for x in num_topks_correct_behavior
                ]
            ...
            for k, topk in zip(ks, behavior_perclass_acc):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["perclass_behavior_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )
            ...
            concat_labels = torch.cat(
                [
                    self.video_labels_animal.view(1, -1),
                    self.video_labels_behavior.view(1, -1),
                ],
                0,
            ).t()
            ...
            for label in unique_composition_labels:
                composition_index = (
                    (concat_labels == label).sum(1) == len(label)
                ).nonzero(as_tuple=True)[0]

                sub_animal_pred = torch.index_select(
                    self.video_preds_animal, 0, composition_index
                )
                sub_animal_label = torch.index_select(
                    self.video_labels_animal, 0, composition_index
                )
                sub_behavior_pred = torch.index_select(
                    self.video_preds_behavior, 0, composition_index
                )
                sub_behavior_label = torch.index_select(
                    self.video_labels_behavior, 0, composition_index
                )
            ...
            for k, topk in zip(ks, composition_perclass_acc):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["perclass_composition_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )
    ...
    def classify_longtail_classes(self, file):
        ...
        for key in composition_dict.keys():
            ...
            if animal_frequency > 300 and behavior_frequency > 1500:
                ...
                animal_frequency <= 300
                and animal_frequency > 100
                and behavior_frequency > 1500
            ):
                am_bh.append(key)
                ...
                animal_frequency > 100
                and animal_frequency <= 300
                and behavior_frequency <= 500
            ):
                am_bf.append(key)
            elif animal_frequency <= 100 and behavior_frequency <= 500:
    ...
    def finalize_metrics_group_by_counterpart(self, ks=(1, 5)):
        ...
        if self.multi_label:
            ...
            for animal in unique_animal_labels:
                ...
                sub_pred = torch.index_select(self.video_preds_animal, 0, label_indexs)
                sub_behavior_gt = torch.index_select(
                    self.video_labels_behavior, 0, label_indexs
                )
                ...
                for index, behavior in enumerate(sub_behavior_gt):
                    ...
                    if behavior not in behavior_dict.keys():
                        behavior_dict[behavior] = [
                            [sub_pred[index].tolist()],
                            [sub_label[index].tolist()],
                        ]
                    else:
            ...
            for behavior in unique_behavior_labels:
                ...
                sub_animal_gt = torch.index_select(
                    self.video_labels_animal, 0, label_indexs
                )
    ...
    def finalize_metrics_long_tail(self, ks=(1, 5)):
        ...
        if self.multi_label:
            ...
            animal_frequency, behavior_frequency, composition_frequency = (
                self.compute_frequency("/train.csv")
            )
            ...
            number_composition_head = number_composition_med = (
                number_composition_tail
            ) = 0
            ...
            number_ah_bh = number_ah_bm = number_ah_bf = number_am_bh = number_am_bm = (
                number_am_bf
            ) = number_af_bh = number_af_bm = number_af_bf = 0
            ...
            with open(self.output_dir + "/animal_prediction.txt", "w") as f:
                for animal in unique_animal_labels:
                    ...
                    sub_pred = torch.index_select(
                        self.video_preds_animal, 0, label_indexs
                    )

                    num_topks_correct_animal, pred_index, gt_index = (
                        metrics.topks_correct(
                            sub_pred, sub_label, ks, return_pred_index=True
                        )
                    )

                    _, pred_index_top1, gt_index_top1 = metrics.topks_correct(
                        sub_pred, sub_label, [1], return_pred_index=True
                    )
                    for each_pred, each_label in zip(pred_index_top1, gt_index_top1):
                        if each_pred.shape[0] > 1:
                            for each_sub_pred, each_sub_label in zip(
                                each_pred, each_label
                            ):
                                f.write(
                                    str(each_sub_pred.item())
                                    + " "
                                    + str(each_sub_label.item())
                                    + "\n"
                                )
                        else:
                            f.write(
                    ...
                    unique_animal_result.append(
                        [
                            animal.item(),
                            topks_animal[0].item(),
                            animal_frequency[str(animal.item())],
                        ]
                    )
            ...
            for k, topk in zip(ks, animal_perclass_acc_head):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["perclass_head_animal_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )
            ...
            for k, topk in zip(ks, animal_perclass_acc_med):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["perclass_med_animal_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )
            ...
            for k, topk in zip(ks, animal_perclass_acc_tail):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["perclass_tail_animal_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )
            ...
            with open(self.output_dir + "/behavior_prediction.txt", "w") as f:
                for behavior in unique_behavior_labels:
                    ...
                    num_topks_correct_behavior, pred_index, gt_index = (
                        metrics.topks_correct(
                            sub_pred, sub_label, ks, return_pred_index=True
                        )
                    )
                    ...
                    if str(torch.unique(sub_label).item()) in behavior_head:
                        ...
                        print(
                            "behavior ",
                            str(
                                torch.unique(sub_label).item(),
                                "do not exist in training",
                            ),
                        )

                    unique_behavior_result.append(
                        [
                            behavior.item(),
                            topks_behavior[0].item(),
                            behavior_frequency[str(behavior.item())],
                        ]
                    )
            ...
            for k, topk in zip(ks, behavior_perclass_acc_head):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["perclass_head_behavior_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )
            ...
            for k, topk in zip(ks, behavior_perclass_acc_med):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["perclass_med_behavior_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )
            ...
            for k, topk in zip(ks, behavior_perclass_acc_tail):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["perclass_tail_behavior_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )
            ...
            for label in unique_composition_labels:
                ...
                animal_behavior = (
                    str(torch.unique(sub_animal_label).item())
                    + "_"
                    + str(torch.unique(sub_behavior_label).item())
                )
                ...
                if label not in composition_frequency:
                    unique_composition_result.append(
                        [label, topks_composition[0].item(), 0]
                    )
                else:
                    unique_composition_result.append(
                        [
                            label,
                            topks_composition[0].item(),
                            composition_frequency[label],
                        ]
                    )
            ...
            for k, topk in zip(ks, composition_perclass_acc_head):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["perclass_head_composition_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )
            ...
            for k, topk in zip(ks, composition_perclass_acc_med):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["perclass_med_composition_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )
            ...
            for k, topk in zip(ks, composition_perclass_acc_tail):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["perclass_tail_composition_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )
    ...
    def finalize_metrics_long_tail_perexample(self, ks=(1, 5)):
        ...
        if self.multi_label:
            ...
            for animal in unique_animal_labels:
                ...
                if str(torch.unique(sub_label).item()) in animal_head:
                    if animal_head_accumulation_pred == None:
                        ...
                        animal_head_accumulation_pred = torch.cat(
                            [animal_head_accumulation_pred, sub_pred], dim=0
                        )

                    if animal_head_accumulation_gt == None:
                        animal_head_accumulation_gt = sub_label
                    else:
                        animal_head_accumulation_gt = torch.cat(
                            [animal_head_accumulation_gt, sub_label], dim=0
                        )
                    ...
                    if animal_med_accumulation_pred == None:
                        ...
                        animal_med_accumulation_pred = torch.cat(
                            [animal_med_accumulation_pred, sub_pred], dim=0
                        )

                    if animal_med_accumulation_gt == None:
                        animal_med_accumulation_gt = sub_label
                    else:
                        animal_med_accumulation_gt = torch.cat(
                            [animal_med_accumulation_gt, sub_label], dim=0
                        )
                    ...
                    if animal_tail_accumulation_pred == None:
                        ...
                        animal_tail_accumulation_pred = torch.cat(
                            [animal_tail_accumulation_pred, sub_pred], dim=0
                        )

                    if animal_tail_accumulation_gt == None:
                        animal_tail_accumulation_gt = sub_label
                    else:
                        animal_tail_accumulation_gt = torch.cat(
                            [animal_tail_accumulation_gt, sub_label], dim=0
                        )
                else:
                    print(sub_label, "do not exist in the training dataset")

            num_topks_correct_animal_head, pred_index, gt_index = metrics.topks_correct(
                animal_head_accumulation_pred,
                animal_head_accumulation_gt,
                ks,
                return_pred_index=True,
            )
            ...
            for k, topk in zip(ks, topks_animal_head):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["animal_head_perexample_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )

            num_topks_correct_animal_med, pred_index, gt_index = metrics.topks_correct(
                animal_med_accumulation_pred,
                animal_med_accumulation_gt,
                ks,
                return_pred_index=True,
            )
            ...
            for k, topk in zip(ks, topks_animal_med):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["animal_med_perexample_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )

            num_topks_correct_animal_tail, pred_index, gt_index = metrics.topks_correct(
                animal_tail_accumulation_pred,
                animal_tail_accumulation_gt,
                ks,
                return_pred_index=True,
            )
            ...
            for k, topk in zip(ks, topks_animal_tail):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["animal_tail_perexample_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )
            ...
            for behavior in unique_behavior_labels:
                ...
                if str(torch.unique(sub_label).item()) in behavior_head:
                    if behavior_head_accumulation_pred == None:
                        ...
                        behavior_head_accumulation_pred = torch.cat(
                            [behavior_head_accumulation_pred, sub_pred], dim=0
                        )

                    if behavior_head_accumulation_gt == None:
                        behavior_head_accumulation_gt = sub_label
                    else:
                        behavior_head_accumulation_gt = torch.cat(
                            [behavior_head_accumulation_gt, sub_label], dim=0
                        )
                    ...
                    if behavior_med_accumulation_pred == None:
                        ...
                        behavior_med_accumulation_pred = torch.cat(
                            [behavior_med_accumulation_pred, sub_pred], dim=0
                        )

                    if behavior_med_accumulation_gt == None:
                        behavior_med_accumulation_gt = sub_label
                    else:
                        behavior_med_accumulation_gt = torch.cat(
                            [behavior_med_accumulation_gt, sub_label], dim=0
                        )
                    ...
                    if behavior_tail_accumulation_pred == None:
                        ...
                        behavior_tail_accumulation_pred = torch.cat(
                            [behavior_tail_accumulation_pred, sub_pred], dim=0
                        )

                    if behavior_tail_accumulation_gt == None:
                        behavior_tail_accumulation_gt = sub_label
                    else:
                        behavior_tail_accumulation_gt = torch.cat(
                            [behavior_tail_accumulation_gt, sub_label], dim=0
                        )

                else:
                    print(
                        "behavior ",
                        str(torch.unique(sub_label).item(), "do not exist in training"),
                    )

            num_topks_correct_behavior_head, pred_index, gt_index = (
                metrics.topks_correct(
                    behavior_head_accumulation_pred,
                    behavior_head_accumulation_gt,
                    ks,
                    return_pred_index=True,
                )
            )
            ...
            for k, topk in zip(ks, topks_behavior_head):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["behavior_head_perexample_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )

            num_topks_correct_behavior_med, pred_index, gt_index = (
                metrics.topks_correct(
                    behavior_med_accumulation_pred,
                    behavior_med_accumulation_gt,
                    ks,
                    return_pred_index=True,
                )
            )
            ...
            for k, topk in zip(ks, topks_behavior_med):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["behavior_med_perexample_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )

            num_topks_correct_behavior_tail, pred_index, gt_index = (
                metrics.topks_correct(
                    behavior_tail_accumulation_pred,
                    behavior_tail_accumulation_gt,
                    ks,
                    return_pred_index=True,
                )
            )
            ...
            for k, topk in zip(ks, topks_behavior_tail):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["behavior_tail_perexample_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )
            ...
            for label in unique_composition_labels:
                ...
                if animal_behavior in composition_head:
                    if (
                        composition_head_accumulation_pred_animal == None
                        or composition_head_accumulation_pred_behavior == None
                    ):
                        composition_head_accumulation_pred_animal = sub_animal_pred
                        composition_head_accumulation_pred_behavior = sub_behavior_pred
                    else:
                        composition_head_accumulation_pred_animal = torch.cat(
                            [
                                composition_head_accumulation_pred_animal,
                                sub_animal_pred,
                            ],
                            dim=0,
                        )
                        composition_head_accumulation_pred_behavior = torch.cat(
                            [
                                composition_head_accumulation_pred_behavior,
                                sub_behavior_pred,
                            ],
                            dim=0,
                        )
                    ...
                    if (
                        composition_head_accumulation_gt_animal == None
                        or composition_head_accumulation_gt_behavior == None
                    ):
                        composition_head_accumulation_gt_animal = sub_animal_label
                        composition_head_accumulation_gt_behavior = sub_behavior_label

                    else:
                        composition_head_accumulation_gt_animal = torch.cat(
                            [composition_head_accumulation_gt_animal, sub_animal_label],
                            dim=0,
                        )
                        composition_head_accumulation_gt_behavior = torch.cat(
                            [
                                composition_head_accumulation_gt_behavior,
                                sub_behavior_label,
                            ],
                            dim=0,
                        )
                    ...
                    if (
                        composition_med_accumulation_pred_behavior == None
                        or composition_med_accumulation_pred_animal == None
                    ):
                        composition_med_accumulation_pred_animal = sub_animal_pred
                        composition_med_accumulation_pred_behavior = sub_behavior_pred
                    else:
                        composition_med_accumulation_pred_animal = torch.cat(
                            [composition_med_accumulation_pred_animal, sub_animal_pred],
                            dim=0,
                        )
                        composition_med_accumulation_pred_behavior = torch.cat(
                            [
                                composition_med_accumulation_pred_behavior,
                                sub_behavior_pred,
                            ],
                            dim=0,
                        )
                    ...
                    if (
                        composition_med_accumulation_gt_behavior == None
                        or composition_med_accumulation_gt_animal == None
                    ):
                        composition_med_accumulation_gt_animal = sub_animal_label
                        composition_med_accumulation_gt_behavior = sub_behavior_label

                    else:
                        composition_med_accumulation_gt_animal = torch.cat(
                            [composition_med_accumulation_gt_animal, sub_animal_label],
                            dim=0,
                        )
                        composition_med_accumulation_gt_behavior = torch.cat(
                            [
                                composition_med_accumulation_gt_behavior,
                                sub_behavior_label,
                            ],
                            dim=0,
                        )
                    ...
                    if (
                        composition_tail_accumulation_pred_animal == None
                        or composition_tail_accumulation_pred_behavior == None
                    ):
                        composition_tail_accumulation_pred_animal = sub_animal_pred
                        composition_tail_accumulation_pred_behavior = sub_behavior_pred
                    else:
                        composition_tail_accumulation_pred_animal = torch.cat(
                            [
                                composition_tail_accumulation_pred_animal,
                                sub_animal_pred,
                            ],
                            dim=0,
                        )
                        composition_tail_accumulation_pred_behavior = torch.cat(
                            [
                                composition_tail_accumulation_pred_behavior,
                                sub_behavior_pred,
                            ],
                            dim=0,
                        )
                    ...
                    if (
                        composition_tail_accumulation_gt_animal == None
                        or composition_tail_accumulation_gt_behavior == None
                    ):
                        composition_tail_accumulation_gt_animal = sub_animal_label
                        composition_tail_accumulation_gt_behavior = sub_behavior_label

                    else:
                        composition_tail_accumulation_gt_animal = torch.cat(
                            [composition_tail_accumulation_gt_animal, sub_animal_label],
                            dim=0,
                        )
                        composition_tail_accumulation_gt_behavior = torch.cat(
                            [
                                composition_tail_accumulation_gt_behavior,
                                sub_behavior_label,
                            ],
                            dim=0,
                        )
            ...
            num_topks_animal, num_toks_behavior, num_topks_correct_composition_head = (
                metrics.topks_correct_composition(
                    composition_head_accumulation_pred_animal,
                    composition_head_accumulation_gt_animal,
                    composition_head_accumulation_pred_behavior,
                    composition_head_accumulation_gt_behavior,
                    ks,
                )
            ...
            for k, topk in zip(ks, topks_composition_head):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["composition_head_perexample_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )

            num_topks_animal, num_toks_behavior, num_topks_correct_composition_med = (
                metrics.topks_correct_composition(
                    composition_med_accumulation_pred_animal,
                    composition_med_accumulation_gt_animal,
                    composition_med_accumulation_pred_behavior,
                    composition_med_accumulation_gt_behavior,
                    ks,
                )
            ...
            for k, topk in zip(ks, topks_composition_med):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["composition_med_perexample_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )
            ...
            num_topks_animal, num_toks_behavior, num_topks_correct_composition_tail = (
                metrics.topks_correct_composition(
                    composition_tail_accumulation_pred_animal,
                    composition_tail_accumulation_gt_animal,
                    composition_tail_accumulation_pred_behavior,
                    composition_tail_accumulation_gt_behavior,
                    ks,
                )
            ...
            for k, topk in zip(ks, topks_composition_tail):
                # self.stats["top{}_acc".format(k)] = topk.cpu().numpy()
                self.stats["composition_tail_perexample_top{}_acc".format(k)] = (
                    "{:.{prec}f}".format(topk, prec=2)
                )
...
class TrainMeter(object):
    ...
    def log_iter_stats(self, cur_epoch, cur_iter, data_type):
        ...
        if data_type == "animal":
            ...
            if not self._cfg.DATA.MULTI_LABEL:
                stats["top1_err_composition"] = (
                    self.mb_top1_err_composition.get_win_median()
                )
                stats["top5_err_composition"] = (
                    self.mb_top5_err_composition.get_win_median()
                )
            logging.log_json_stats(stats)
