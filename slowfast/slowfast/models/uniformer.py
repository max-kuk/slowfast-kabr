...
model_path = {
    "uniformer_small_k400_16x4": os.path.join(
        model_path, "uniformer_small_k400_16x4.pth"
    ),
    "uniformer_small_k600_16x4": os.path.join(
        model_path, "uniformer_small_k600_16x4.pth"
    ),
    "uniformer_base_k400_16x4": os.path.join(
        model_path, "uniformer_base_k400_16x4.pth"
    ),
    "uniformer_base_k600_16x4": os.path.join(
        model_path, "uniformer_base_k600_16x4.pth"
    ),
    "uniformer_base_k400_32x4": os.path.join(
        model_path, "uniformer_base_k400_32x4.pth"
    ),
    "uniformer_base_k600_32x4": os.path.join(
        model_path, "uniformer_base_k600_32x4.pth"
    ),
}
    ...
    def get_pretrained_model(self, cfg):
        if cfg.UNIFORMER.PRETRAIN_NAME:
            checkpoint = torch.load(
                model_path[cfg.UNIFORMER.PRETRAIN_NAME], map_location="cpu"
            )
            ...
            for k in checkpoint.keys():
                if checkpoint[k].shape != state_dict_3d[k].shape:
                    if len(state_dict_3d[k].shape) <= 2:
                        ...
                        continue
                    logger.info(
                        f"Inflate: {k}, {checkpoint[k].shape} => {state_dict_3d[k].shape}"
                    )
