...
def load_fasttext_embeddings_mammalnet(vocab):
    vocab_lower = [v.lower() for v in vocab]
    vocab = []
    for current in vocab_lower:
        vocab.append(current)
    ...
    ft = fasttext.load_model(
        "/ibex/ai/project/c2133/benchmarks/mvit2/SlowFast/slowfast/models/fasttext_pretrained/cc.en.300.bin"
    )
    embeds = []
    for k in vocab:
        if "_" in k:
...
def test_word_embedding():
    ...
    with open(
        "/ibex/ai/project/c2133/benchmarks/mvit2/SlowFast/animalnet_trimmed_last/genus_to_id.pkl",
        "rb",
    ) as f:
        data = pkl.load(f)
    # for key in data.keys():
    feature = load_fasttext_embeddings_mammalnet(data.keys())
    print(feature.shape)
