...
class Uniformerv2(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.UNIFORMERV2.PRETRAIN != "":
            ...
            if (
                cfg.UNIFORMERV2.DELETE_SPECIAL_HEAD
                and state_dict["backbone.transformer.proj.2.weight"].shape[0]
                != num_classes
            ):
                ...
                if (
                    num_classes == 400
                    or state_dict["backbone.transformer.proj.2.weight"].shape[0]
                    == num_classes
                ):
                    state_dict["backbone.transformer.proj.2.weight"] = state_dict[
                        "backbone.transformer.proj.2.weight"
                    ][:num_classes]
                    state_dict["backbone.transformer.proj.2.bias"] = state_dict[
                        "backbone.transformer.proj.2.bias"
                    ][:num_classes]
                else:
