...
from .word_embedding import load_fasttext_embeddings_mammalnet
...
@MODEL_REGISTRY.register()
class SlowFast_composition_czsl(nn.Module):
    """
    SlowFast model builder for SlowFast network.
    ...
    def __init__(self, cfg):
        ...
        Args:
            cfg (CfgNode): model building configs, details are in the
                comments of the config file.
        """
        super(SlowFast_composition_czsl, self).__init__()
        self.norm_module = get_norm(cfg)
        self.cfg = cfg
        self.enable_detection = cfg.DETECTION.ENABLE
        self.num_pathways = 2
        ...
        while tag:
            try:
                with open(
                    "/jmain02/home/J2AD001/wwp02/mxk71-wwp02/uob_project/mammalnet/annotation/genus_to_id.txt",
                    "rb",
                ) as f:
                    id_to_genus = pkl.load(f)
                    for key in id_to_genus.keys():
                        animals.append(id_to_genus[key])
                        animals_index.append(key)
                    tag = False
            except:
                time.sleep(0.1)
        ...
        while tag:
            try:
                with open(
                    "/jmain02/home/J2AD001/wwp02/mxk71-wwp02/uob_project/mammalnet/annotation/behavior_to_id.txt",
                    "rb",
                ) as f:
                    id_to_behavior = pkl.load(f)
                    for key in id_to_behavior.keys():
                        behaviors.append(id_to_behavior[key])
                        behavior_index.append(key)
        ...
        # animals, behaviors = zip(*full_pairs)

        animal_emb = load_fasttext_embeddings_mammalnet(animals)
        behavior_emb = load_fasttext_embeddings_mammalnet(behaviors)

        # print("animal emb shape", animal_emb.shape)
        # print("behavior emb shape", behavior_emb.shape)

        self.animal_index = torch.tensor(animals_index).cuda()
        self.behavior_index = torch.tensor(behavior_index).cuda()

        self.animal_tensor = torch.tensor(animal_emb, dtype=torch.float32).cuda()
        self.behavior_tensor = torch.tensor(behavior_emb, dtype=torch.float32).cuda()

        self.animal_embedder = nn.Embedding(animal_emb.shape[0], animal_emb.shape[1])
        self.behavior_embedder = nn.Embedding(
            behavior_emb.shape[0], behavior_emb.shape[1]
        )

        # self.animal_embedder.weight.data.copy_(animal_emb)
        # self.behavior_embedder.weight.data.copy_(behavior_emb)

        self.all_animals = animals
        self.all_behaviors = behaviors
    ...
    def _construct_network(self, cfg):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            # self.linear_behavior_head = nn.Linear(400,300, bias=True)
            # self.linear_animal_head = nn.Linear(400,300,bias=True)

            self._behavior_head = head_helper.ResNetBasicHead(
                dim_in=[
                    width_per_group * 32,
                    width_per_group * 32 // cfg.SLOWFAST.BETA_INV,
                ],
            ...
            self._animal_head = head_helper.ResNetBasicHead(
                dim_in=[
                    width_per_group * 32,
                    width_per_group * 32 // cfg.SLOWFAST.BETA_INV,
                ],
    ...
    def forward(self, x, animal_label=None, behavior_label=None, bboxes=None):
        x = x[:]  # avoid pass by reference
        x = self.s1(x)
        x = self.s1_fuse(x)
        x = self.s2(x)
        ...
        behavior_prediction = self._behavior_head(x)
        animal_prediction = self._animal_head(x)

        animal_prediction = F.normalize(animal_prediction, dim=1)
        behavior_prediction = F.normalize(behavior_prediction, dim=1)

        animal_tensor = F.normalize(
            self.animal_embedder(self.animal_index), dim=1
        ).permute(1, 0)
        behavior_tensor = F.normalize(
            self.behavior_embedder(self.behavior_index), dim=1
        ).permute(1, 0)

        animal_prediction = torch.matmul(animal_prediction, animal_tensor)
        behavior_prediction = torch.matmul(behavior_prediction, behavior_tensor)

        return behavior_prediction, animal_prediction


@MODEL_REGISTRY.register()
class SlowFast_composition(nn.Module):
    """
    SlowFast model builder for SlowFast network.
    ...
    def __init__(self, cfg):
        ...
        Args:
            cfg (CfgNode): model building configs, details are in the
                comments of the config file.
        """
        super(SlowFast_composition, self).__init__()
        self.norm_module = get_norm(cfg)
        self.cfg = cfg
        self.enable_detection = cfg.DETECTION.ENABLE
        self.num_pathways = 2
    ...
    def _construct_network(self, cfg):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            self._behavior_head = head_helper.ResNetBasicHead(
                ...
                num_classes=cfg.MODEL.NUM_CLASSES_BEHAVIOR,
                pool_size=[None, None]
                if cfg.MULTIGRID.SHORT_CYCLE
                or cfg.MODEL.MODEL_NAME == "ContrastiveModel"
                else [
            ...
            self._animal_head = head_helper.ResNetBasicHead(
                ...
                num_classes=cfg.MODEL.NUM_CLASSES_ANIMAL,
                pool_size=[None, None]
                if cfg.MULTIGRID.SHORT_CYCLE
                or cfg.MODEL.MODEL_NAME == "ContrastiveModel"
                else [
...
@MODEL_REGISTRY.register()
class ResNet_composition(nn.Module):
    """
    ResNet model builder. It builds a ResNet like network backbone without
    lateral connection (C2D, I3D, Slow).
    ...
    def __init__(self, cfg):
        ...
        Args:
            cfg (CfgNode): model building configs, details are in the
                comments of the config file.
        """
        super(ResNet_composition, self).__init__()
        self.norm_module = get_norm(cfg)
        self.enable_detection = cfg.DETECTION.ENABLE
        self.num_pathways = 1
        self._construct_network(cfg)
