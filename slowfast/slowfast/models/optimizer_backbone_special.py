...
def construct_optimizer(model, cfg):
    ...
    optim_params = [
        ...
        {
            "params": special_bn_parameters,
            "weight_decay": cfg.BN.WEIGHT_DECAY,
            "lr": cfg.SOLVER.BASE_LR * cfg.SOLVER.SPECIAL_RATIO,
        },
        {
            "params": special_non_bn_parameters,
            "weight_decay": cfg.SOLVER.WEIGHT_DECAY,
            "lr": cfg.SOLVER.BASE_LR * cfg.SOLVER.SPECIAL_RATIO,
        },
        {
            "params": special_zero_parameters,
            "weight_decay": 0.0,
            "lr": cfg.SOLVER.BASE_LR * cfg.SOLVER.SPECIAL_RATIO,
        },
    ]
    ...
    assert total_num == len(non_bn_parameters) + len(special_non_bn_parameters) + len(
        bn_parameters
    ) + len(special_bn_parameters) + len(zero_parameters) + len(
        special_zero_parameters
    ...
    logger.info(
        "SPECIAL: bn {}, non bn {}, zero {}, lr {}".format(
            len(special_bn_parameters),
            len(special_non_bn_parameters),
            len(special_zero_parameters),
