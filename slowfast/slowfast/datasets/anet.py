...
class Anet(torch.utils.data.Dataset):
    """
    ANet video loader. Construct the ANet video loader, then sample
    ...
    def __init__(self, cfg, mode, num_retries=10):
        """
        Construct the ANet video loader with a given csv file. The format of
        the csv file is:
        ```
        path_to_video_1 total_time_1 start_time_1 end_time_1 label_1
        path_to_video_2 total_time_2 start_time_2 end_time_2 label_2
        ...
        path_to_video_N total_time_N start_time_N end_time_N label_N
        ```
        Args:
            cfg (CfgNode): configs.
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for ANet".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        logger.info("Constructing ANet {}...".format(mode))
    ...
    def _construct_loader(self):
        ...
        with g_pathmgr.open(path_to_file, "r") as f:
            for clip_idx, path_label in enumerate(f.read().splitlines()):
                ...
                #     == 5
                # )
                path, total_time, start_time, end_time, label = path_label.split(
                ...
                for idx in range(self._num_clips):
                    ...
                    self._total_time.append(float(total_time))
                    self._start_time.append(float(start_time))
                    self._end_time.append(float(end_time))
                    self._labels.append(int(label))
                    self._spatial_temporal_idx.append(idx)
                    self._video_meta[clip_idx * self._num_clips + idx] = {}
        assert (
            len(self._path_to_videos) > 0
        ), "Failed to load ANet split {} from {}".format(self._split_idx, path_to_file)
        logger.info(
            "Constructing ANet dataloader (size: {}) from {}".format(
    ...
    def __getitem__(self, index):
        ...
        for i_try in range(self._num_retries):
            ...
            # read video for ANet
            total_time = self._total_time[index]
            start_time = self._start_time[index]
            end_time = self._end_time[index]
            total_frames = len(video_container) / total_time * (end_time - start_time)
            start_index = int(len(video_container) / total_time * start_time)
