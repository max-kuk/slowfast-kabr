...
def save_checkpoint(path_to_job, model, optimizer, loss_scaler, epoch, cfg, zip=True):
    ...
    path_to_checkpoint = get_path_to_checkpoint(
        path_to_job, epoch + 1, save_latest=cfg.TRAIN.SAVE_LATEST
    )
...
def save_best_checkpoint(
    path_to_job, model, optimizer, loss_scaler, epoch, cfg, zip=True, name="best.pyth"
):
    """
    Save a checkpoint.
    Args:
...
def load_checkpoint(
    ...
    """
    assert g_pathmgr.exists(path_to_checkpoint), "Checkpoint '{}' not found".format(
        path_to_checkpoint
    )
    ...
    if convert_from_caffe2:
        ...
        model_state = normal_to_sub_bn(model_state, model_state_dict_3d)
        if inflation:
            # Try to inflate the model.
            inflated_model_dict = inflate_weight(model_state, model_state_dict_3d)
...
def load_train_checkpoint(cfg, model, optimizer, loss_scaler):
    ...
    if cfg.TRAIN.AUTO_RESUME and has_checkpoint(cfg.OUTPUT_DIR):
        last_checkpoint = get_last_checkpoint(
            cfg.OUTPUT_DIR, save_latest=cfg.TRAIN.SAVE_LATEST
        )
    ...
    elif cfg.TEST.TEST_BEST and g_pathmgr.exists(
        os.path.join(cfg.OUTPUT_DIR, "best.pyth")
    ):
