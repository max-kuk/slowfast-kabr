    ...
    for cur_iter, (inputs, labels, behavior_labels, index, time, meta) in enumerate(
        ...
        if cfg.NUM_GPUS:
            ...
            if not isinstance(labels, list):
                ...
                behavior_labels = behavior_labels.cuda(non_blocking=True)
        ...
        if cfg.MIXUP.ENABLE:
            print(inputs[0], labels, behavior_labels)
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            ...
            if cfg.MODEL.MODEL_NAME == "ContrastiveModel" and partial_loss:
                ...
                # Compute the loss.
                loss = 0.5 * loss_fun(animal_prediction, labels) + 0.5 * loss_fun(
...
def build_trainer(cfg):
    ...
    val_loader = loader_composition.construct_loader(cfg, "val")
    precise_bn_loader = loader_composition.construct_loader(
...
def train(cfg):
    ...
    val_loader = loader_composition.construct_loader(cfg, "val")
    precise_bn_loader = (
        loader_composition.construct_loader(cfg, "train", is_precise_bn=True)
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        if cur_epoch > 0 and cfg.DATA.LOADER_CHUNK_SIZE > 0:
            ...
            train_loader = loader_composition.construct_loader(cfg, "train")
            loader_composition.shuffle_dataset(train_loader, cur_epoch)
